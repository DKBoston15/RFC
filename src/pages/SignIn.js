...
const SignIn = () => {
    ...
    const { signIn, signUpGoogle, resetPassword } = useAuth()
    const [signUpFormDisplay, setSignUpFormDisplay] = useState(false)
    const [passwordResetSent, setPasswordResetSent] = useState(false)
    const [passwordResetDisplay, setPasswordResetDisplay] = useState(false)
    const [email, setEmail] = useState()
    const [password, setPassword] = useState()
    const [headerText, setHeaderText] = useState("Sign in to your RFC Account")
    ...
    return (
        <Box className={classes.root}>
            ...
            {!signUpFormDisplay && !passwordResetDisplay && (
                <Button
            ...
            {passwordResetDisplay && (
                <>
                    ...
                    {passwordResetSent && (
                        <Typography variant="subtitle1">
                            If an account under this email exists, you will
                            receive a password reset email. You can close this
                            page.
                        </Typography>
                    )}
                </>
